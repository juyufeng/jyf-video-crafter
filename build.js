...
const filePath = path.join(__dirname, 'es6/JyfVideoCrafter.js');

console.log(chalk`{blue.underline begin ejs}`);
run(`npx babel JyfVideoCrafter.js  --out-dir ejs`)
console.log(chalk`{blue.underline finish ejs}`);

console.log(chalk`{blue.underline begin es6}`);
run(`npx babel JyfVideoCrafter.js  --out-dir es6`)
fs.readFile(filePath, 'utf8', (err, data) => {
  ...
  'use strict';`;
  const B = `  return JyfVideoCrafter;
});`;
  ...
  const modifiedData = match[1] + `export default JyfVideoCrafter;`;
