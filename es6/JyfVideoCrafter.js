  ...
  // -----------------
  var ArrayBufferDataStream = function ArrayBufferDataStream(length) {
  ...
  var BlobBuffer = function BlobBuffer() {
            ...
            return new Promise(function (resolve, reject) {
              convertToUint8Array(newEntry.data).then(function (dataArray) {
                var totalWritten = 0,
                  ...
                  handleWriteComplete = function handleWriteComplete(err, written, buffer) {
            ...
            var _loop = function _loop() {
                ...
                if (!(newEntry.offset + newEntry.length <= entry.offset || newEntry.offset >= entry.offset + entry.length)) {
                  ...
                  if (newEntry.offset == entry.offset && newEntry.length == entry.length) {
                    ...
                    return {
                      v: convertToUint8Array(entry.data).then(function (entryArray) {
  ...
  function decodeBase64WebPDataURL(url) {
    if (kksTp(url) !== "string" || !url.match(/^data:image\/webp;base64,/i)) {
  ...
  function renderAsWebP(canvas, quality) {
    var frame = kksTp(canvas) === 'string' && /^data:image\/webp/.test(canvas) ? canvas : canvas.toDataURL('image/webp', quality);
  ...
  function extractKeyframeFromWebP(webP) {
    var cursor = webP.indexOf('VP8', 12); // Start the search after the 12-byte file header
  ...
  function writeEBML(buffer, bufferFileOffset, ebml) {
    ...
    if (Array.isArray(ebml)) {
      ...
      if (Array.isArray(ebml.data)) {
      ...
      } else if (kksTp(ebml.data) === "string") {
        buffer.writeEBMLVarInt(ebml.data.length); // Size field
        ebml.dataOffset = buffer.pos + bufferFileOffset;
        buffer.writeString(ebml.data);
      } else if (kksTp(ebml.data) === "number") {
        ...
        throw new Error("Bad EBML datatype " + kksTp(ebml.data));
      }
    } else {
      throw new Error("Bad EBML datatype " + kksTp(ebml.data));
    }
   ...
   */
  var JyfVideo = function JyfVideo(ArrayBufferDataStream, BlobBuffer) {
    return function (options) {
      var MAX_CLUSTER_DURATION_MSEC = 5000,
        ...
        ebmlSegment,
        // Root element of the EBML document
      ...
      function convertAlphaToGrayscaleImage(source) {
        ...
        for (var srcCursor = 3 /* Since pixel byte order is RGBA */; srcCursor < srcEnd; srcCursor += 4) {
          var alpha = sourceData[srcCursor];
      ...
      function flushClusterFrameBuffer() {
        ...
        var buffer = new ArrayBufferDataStream(rawImageSize + clusterFrameBuffer.length * 64),
          // Estimate 64 bytes per block header

          cluster = createCluster({
            timecode: Math.round(clusterStartTime)
          });
        for (var _i = 0; _i < clusterFrameBuffer.length; _i++) {
          cluster.data.push(createContainerForKeyframe(clusterFrameBuffer[_i]));
        }
      ...
      function rewriteSegmentLength() {
        var buffer = new ArrayBufferDataStream(10),
          oldPos = blobBuffer.pos;
      ...
      this.addFrame = function (frame, alpha, overrideFrameDuration) {
        ...
        var keyframe = extractKeyframeFromWebP(renderAsWebP(frame, options.quality)),
          frameDuration,
          frameAlpha = null;
        if (overrideFrameDuration) {
          frameDuration = overrideFrameDuration;
        } else if (kksTp(alpha) == "number") {
          frameDuration = alpha;
        } else {
          frameDuration = options.frameDuration;
        }
        if (options.transparent) {
          if (alpha instanceof HTMLCanvasElement || kksTp(alpha) === "string") {
            frameAlpha = alpha;
  ...
  var JyfVideoCrafter = new JyfVideo(ArrayBufferDataStream, BlobBuffer());
export default JyfVideoCrafter;